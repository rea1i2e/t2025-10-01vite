...
| Splide スライダー | `src/assets/js/demo/_splide-fade.js` 等（`_splide-loop`、`_splide-thumbnail`、`_splide-spotlight`、`_splide-progress`、`_splide-segment`、`_splide-variable-size`、`_splide-posts`） | 同名 `_p-splide-*.ejs` | 同名 `_p-splide-*.scss` | `src/demo/demo-splide/index.html` |
...
| `ai-docs/session-log.md` | Cursor ↔ Claude Code 作業引き継ぎログ |
...
- **反映先**: `src/assets/sass/base/_root.scss`、`src/ejs/common/_head.ejs` の preload（FV 用 1 ファイルのみ）
