...
案件で XD / デザインから画像を置くときの**パス正本**。実装の画像パスと揃えて追記する。
...
- テンプレ同梱のデモ画像（`demo/`）は使わない。マークアップには本番予定パスを先に書く
...
| **ファイル圧縮** | JPEG/PNG 等の再エンコード | **Vite ビルド**（`config/site.config.js` の imagemin 等）。配置時に余計な再圧縮をしない |
