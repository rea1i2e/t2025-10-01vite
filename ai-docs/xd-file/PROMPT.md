...
XD エクスポート（JSON + PNG）を手がかりに、EJS / HTML / SCSS を更新するときの**作業手順**。  
...
0. **デザイントークン・フォント・レイアウト幅（セクション実装の前・必須）**
   ...
   - preload は FV 用 **1 ファイルのみ**（`src/ejs/common/_head.ejs` の `/assets/fonts/…`）。
...
4. **レイアウト情報の整理**
   ...
   - 小さめのレイアウトは「共通パーツ」と判断し、既存コンポーネント・デモを優先して再利用する。
